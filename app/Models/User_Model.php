        ...
        if ($roles != 'Operator') {
           ...
           // $build->where('site_id',$Data['Site_ID']);
            ...
            if($build->update()){
                    ...
                    $build1 = $db->table('user_access_control');
                    if (!empty($roles['Financial_Drill_Down'])) {
                        $build1->set('oee_financial_drill_down', $roles['Financial_Drill_Down']);
                    }
                    if (!empty($roles['Financial_Opportunity_Insights'])) {
                        $build1->set('opportunity_insights', $roles['Financial_Opportunity_Insights']);
                    }
                    if (!empty($roles['OEE_Drill_Down'])) {
                        $build1->set('oee_drill_down', $roles['OEE_Drill_Down']);
                    }
                    if (!empty($roles['Operator_User_Interface'])) {
                        $build1->set('operator_user_interface', $roles['Operator_User_Interface']);
                    }
                    if (!empty($roles['Production_Data_Management'])) {
                        $build1->set('production_data_management', $roles['Production_Data_Management']);
                    }
                    if (!empty($roles['Settings_Machine'])) {
                        $build1->set('settings_machine', $roles['Settings_Machine']);
                    }
                    if (!empty($roles['Settings_Parts'])) {
                        $build1->set('settings_part', $roles['Settings_Parts']);
                    }
                    if (!empty($roles['Settings_General'])) {
                        $build1->set('settings_general', $roles['Settings_General']);
                    }
                    if (!empty($roles['Settings_User_Management'])) {
                        $build1->set('settings_user_management', $roles['Settings_User_Management']);
                    }
                    if (!empty($roles['Last_Updated_By'])) {
                        $build1->set('last_updated_by', $roles['Last_Updated_By']);
                    }

                    $build1->where('user_id', $roles['user_id']);
